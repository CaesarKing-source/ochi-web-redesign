    ...
    <div data-scroll data-scroll-section data-scroll-speed='0.2' className="w-full h-screen bg-zinc-900 p-20 flex justify-between">
        <div className="w-1/2 h-full flex flex-col justify-between">
            ...
            transition={{ease: [0.76, 0, 0.24, 1], duration: 1}} className="footer-heading">
                <h1 className="text-9xl leading-none tracking-tighter font-bold uppercase">Eye-</h1>
                <h1 className="text-9xl leading-none tracking-tighter font-bold uppercase">Opening</h1>
            </motion.div>
            <div className="logo">
        ...
        <div className="w-1/2">
            <div className="footer-heading">
                <motion.h1 initial={{ opacity: 0, y: "100px" }} whileInView={{ opacity: 1, y: "0" }}
            transition={{ease: [0.76, 0, 0.24, 1], duration: 1}} className="text-8xl leading-none tracking-tighter font-bold uppercase">
                Presentations</motion.h1>
            </div>
            <motion.div initial={{ opacity: 0, y: "100px" }} whileInView={{ opacity: 1, y: "0" }}
            transition={{ease: [0.76, 0, 0.24, 1], duration: 1}} className="dets font-['Neue_Montreal'] flex flex-col mt-4">
                <p className="text-lg font-light my-4">S"</p>
                {['Instagram', 'Behance', 'Facebook', 'Twitter'].map((link, index) => <a key={index} className="text-lg font-light underline" href="#">{link}</a>)}
            </motion.div >

            <motion.div initial={{ opacity: 0, y: "100px" }} whileInView={{ opacity: 1, y: "0" }}
            transition={{ease: [0.76, 0, 0.24, 1], duration: 1}} className="dets font-['Neue_Montreal'] flex flex-col mt-4">
                <p className="text-lg font-light my-4">L"</p>
                {['202-1965 W 4th Avenue', 'Vancouver, Canada'].map((link, index) => <a key={index} className="text-lg font-light underline" href="#">{link}</a>)}
            </motion.div >

            <motion.div initial={{ opacity: 0, y: "100px" }} whileInView={{ opacity: 1, y: "0" }}
            transition={{ease: [0.76, 0, 0.24, 1], duration: 1}} className="dets font-['Neue_Montreal'] flex flex-col mt-4">
                <p className="text-lg font-light my-4">L"</p>
                <a href="mailto:hello@ochi.design" className="text-lg font-light underline">
                hello@ochi.design
                </a>
            </motion.div >
