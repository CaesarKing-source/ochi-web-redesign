...
const Preloader = () => {
  ...
  useEffect(() => {
    ...
    if (totalCount === 0) {
      ...
      for (let i = 0; i < totalCount; i++) {
        const img = images[i];
        if (img.complete) handleAssetLoaded();
        else img.addEventListener("load", handleAssetLoaded);
      }
    }
  ...
  return (
    <div className="preloader fixed inset-0 z-[9999] bg-zinc-900 flex flex-col items-center justify-center text-white">
      <div className="flex flex-col items-center">
        <div className="logo">
            <svg width="100" height="50" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <h1 className="text-[2.5vw] text-[#CDEA68] font-bold uppercase tracking-widest mb-4">
          Presentation Design Agency
        </h1>

        <div className="text-[2vw] font-light tracking-wider">
          {progress < 100 ? `${progress}%` : "100%"}
        </div>

        <div className="w-[200px] h-[3px] bg-zinc-700 mt-6 overflow-hidden rounded-full">
          <div
            className="h-full bg-[#CDEA68]"
            style={{ width: `${progress}%`, transition: "width 0.3s ease" }}
          ></div>
        </div>
      </div>

      <p className="text-sm absolute bottom-10">Made with 💖 . Akshay Giri</p>
